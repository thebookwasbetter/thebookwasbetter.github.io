---
layout: post
title: Circuit Plushy
subtitle: circuit plushy process 
tags: [blog post]
comments: true
---

### Who's the Plushie?

This plushy is an example of circuitry using conductive thread on a felt fabric. The determiner of if the circuit is viable is the beautiful lights that shine when you press the button and have the switch turned on, closing the circuit!

### From Prototype to Photons

![paperproto](https://thebookwasbetter.github.io/assets/img/paperproto.png)

![alligator](https://thebookwasbetter.github.io/assets/img/alligator.png)

![front](https://thebookwasbetter.github.io/assets/img/front.png)

![back](https://thebookwasbetter.github.io/assets/img/back.png)

### Tip for Pastbrina

Start with a wider breadth between cross stitches. Also, be mindful of what parts have +/- charges!
